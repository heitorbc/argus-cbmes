...
function fromDetalhe(detalhe: ViaturaDetalhe): FormState {
  ...
  return {
    ...
    estadoTanquePercent: i?.estadoTanquePercent !== undefined ? String(i.estadoTanquePercent) : '',
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <section className="mx-auto max-w-3xl space-y-4 p-4">
        {loading && !detalhe && <p className="text-center text-sm text-slate-500">Carregando…</p>}
        {error && (
          <div
            role="alert"
            ...
            {form && (
              ...
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-base font-semibold text-cbmes-blue">Dados operacionais</h2>
                  {!isAdmin && (
                ...
                <div>
                  ...
                  >
                    <option value="">{mfRecursos.length === 0 ? 'Sem dados do MF' : '—'}</option>
                    {mfRecursos.map((r) => (
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <label className="block">
                    <span className="text-sm font-medium text-slate-700">Estado do tanque (%)</span>
                    <input
                      type="number"
                      min={0}
                      max={100}
                ...
                {saveMsg && (
                  <p
                    role="alert"
                    className={`text-sm ${
                      saveMsg === 'Alterações salvas.' ? 'text-emerald-700' : 'text-feedback-error'
                    }`}
                  >
                    {saveMsg}
                  </p>
...
function Linha({ rotulo, valor, badge }: { rotulo: string; valor?: string; badge?: string }) {
  ...
  return (
    <div className="flex items-baseline justify-between gap-2 border-b border-slate-100 py-1 last:border-0">
      ...
      <dd className="text-right text-sm font-medium text-slate-800">
        {badge ? (
          <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${badge}`}>{valor}</span>
        ) : (
          valor
        )}
      </dd>
